...
text_to_remove = 'some text'

def remove_watermark(path):
    print('Removing Watermark')
    watermarked_img = cv2.imread(path)
    _, thresh = cv2.threshold(watermarked_img, 150, 255, cv2.THRESH_BINARY)
    # cv2.imshow('Result', thresh)
    cv2.imwrite(f'{path.replace("watermarked","non_watermarked")}', thresh)
    print('Removing Watermark Completed')
    return f'{path.replace("watermarked","non_watermarked")}'
# cv2.waitKey(0)
...
def inpaint_text(img_path, pipeline):
    ...
    for box in prediction_groups[0]:
        if str(box[0]).lower() not in text_to_remove.split(' '):
            continue
        x0, y0 = box[1][0]
        x1, y1 = box[1][1]
        x2, y2 = box[1][2]
...
path = r'image/path.jpeg'
img_text_removed = inpaint_text(path, pipeline)
plt.imshow(img_text_removed)
cv2.imwrite(f'output_{path}', cv2.cvtColor(img_text_removed, cv2.COLOR_BGR2RGB))
remove_watermark(f'output_{path}')